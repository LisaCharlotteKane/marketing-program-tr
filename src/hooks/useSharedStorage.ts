        ...
        try {
          if (typeof window !== 'undefined') {
            ...
            if (currentSize + newSize > STORAGE_LIMITS.localStorage) {
              notify.error('Storage limit exceeded. Please clear some data or export to CSV.');
              return;
            }
            
            // Warn if approaching limit
            if (currentSize + newSize > STORAGE_LIMITS.warning) {
              notify.warning('Storage is getting full. Consider exporting data to prevent issues.');
            }
          ...
          if (error instanceof DOMException && error.name === 'QuotaExceededError') {
            notify.error('Browser storage is full. Please clear storage in Settings to continue.');
          } else {
            notify.error('Failed to save data. Storage may be corrupted.');
          }
        }
      }, 300); // 300ms debounce
